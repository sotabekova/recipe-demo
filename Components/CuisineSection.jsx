...
export default function CuisineSection() {
  const cuisines = [
    ...
    { name: "Indian", image: "/images/Indianbg.png", link: "/indian" },
    ...
    <div className="w-full pb-20 px-8 flex justify-center">
      <div className="w-full max-w-6xl">
        <div className="text-center mb-12">
          <h2 className="text-amber-950 text-4xl md:text-6xl font-bold mb-3">
            Browse by Cuisine
          </h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          {cuisines.map((cuisine, index) => (
            <Link key={index} to={cuisine.link} className="group cursor-pointer">
              ...
              <p className="text-center mt-3 text-amber-950 font-semibold text-lg">
                {cuisine.name}
              </p>
            </Link>
          ))}
